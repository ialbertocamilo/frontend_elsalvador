...
const LocationSearch = ({placeholder, goToPlace}) => {
    ...
    useEffect(() => {
        document.addEventListener('click', (e) => {
            if (!resultBox.current?.contains(e.target as Node) && !searcher.current?.contains(e.target as Node)) setSearchData([])
    ...
    return (
        <>
            ...
            {searchData?.length ? (
                <Card ref={resultBox} className={"position-absolute z-3 w-100 mt-5"}
