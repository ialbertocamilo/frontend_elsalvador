...
const Row = ({data, onInputChange, onRemove}) => {
    return (
        <tr>
            <td className='p-2'>
                <Input
            ...
            <td className='p-2'>
                <FormGroup id='width-window'>
                    <div className='d-flex align-content-between'>
                        <Button color='storybook' onClick={(e) => onRemove(e)}>-</Button>
                    </div>
                </FormGroup>
            </td>
        </tr>
...
export const ProportionTable = ({onData, keyName}) => {
    ...
    useEffect(() => {
        const resultData = {totalOpaqueSurface: totalOpaqueSurface, totalGlazedSurface: totalGlazedSurface, totalPercentage: totalPercentage}
    ...
    useEffect(() => {
        projects.getProjectData({key: keyName, project_id: params?.projectId as string}).then(res => {
    ...
    const handleInputChange = (index: string | number, column: string | number, val: any) => {
        ...
        for (const newRowsKey in newRows) {
            ...
            newRows[newRowsKey]['column4'] = Number(calculatePercentage(newRows[newRowsKey]['column3'], newRows[newRowsKey]['column2']));
        }
        setRow(newRows);
